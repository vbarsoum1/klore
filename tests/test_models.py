...
def test_default_fast_model(tmp_path: Path) -> None:
    """get_model('fast', ...) returns google/gemini-2.5-flash with no config."""
    assert get_model("fast", tmp_path) == "google/gemini-2.5-flash"


def test_default_strong_model(tmp_path: Path) -> None:
    """get_model('strong', ...) returns anthropic/claude-sonnet-4-6 with no config."""
...
def test_config_override_model(tmp_path: Path) -> None:
    ...
    config_dir.mkdir()
    config = {"model": {"fast": "openai/gpt-4o", "strong": "anthropic/claude-opus-4-6"}}
    (config_dir / "config.json").write_text(json.dumps(config))

    assert get_model("fast", tmp_path) == "openai/gpt-4o"
    assert get_model("strong", tmp_path) == "anthropic/claude-opus-4-6"
...
def test_context_limit_known_model() -> None:
    ...
    assert get_context_limit("google/gemini-2.5-flash") == 1_048_576
